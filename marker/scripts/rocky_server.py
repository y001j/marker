...
class CommonParams(BaseModel):
    ...
    use_llm: Annotated[
        Optional[bool],
        Field(
            description="Use llm to support hybrid mode to convert pdf.")
    ] = False
...
async def _convert_pdf(params: CommonParams):
    ...
    try:
        ...
        out_folder = config_parser.get_output_folder(params.filepath)
        save_output(rendered, out_folder, "output")
...
def server_rocky(port: int, host: str):
    import uvicorn
    # Run the server
    uvicorn.run(
        app,
